...
# -*- coding: utf-8 -*-
"""
red_keyword_spotting_curriculum.py
...
Stage 1 (easy data):
--------------------
- Inputs: pure RED repetitions
    * "5 times Red.zip"
    * "Red 100x 10Hz.zip"
    * "Red 100x 13Hz.zip"
  plus non-RED sentences:
    * "not_RED_speaking.zip"
- Detect RED pulses via energy peaks in pure-RED recordings.
- Extract fixed-length segments around each pulse (positives).
- Extract random segments from non-RED sentences (negatives).
- Train a 2D CNN on log-power spectrograms of these segments.
- This teaches the network a clean RED pattern.

Stage 2 (harder data, curriculum fine-tuning):
----------------------------------------------
- Inputs: sentence-level recordings
    * "Red_story_231125.zip"  (sentences with RED)
    * "not_RED_speaking.zip"  (sentences without RED)
- Use the Stage-1 RED template and normalized cross-correlation (NCC)
  to locate RED centers within each RED sentence (weak supervision).
- Extract segments around those centers (positives) and from other
  regions / non-RED sentences (negatives).
- Fine-tune the same CNN on this harder dataset.

Stage 3 (dynamic detection & stability analysis):
-------------------------------------------------
- For held-out test sentences (with and without RED), slide a window
  across time and compute p(RED | segment) from the CNN.
- Visualizations:
    * Waveform + probability p(RED | t) over time (dynamic detection).
    * Histogram of max p(RED) for RED vs non-RED sentences.
    * Overlay of CNN-detected RED segments (waveform) and their
      average spectrogram (stability of the RED pattern).

All plots are saved under a directory (default: "plots_red_curriculum/").
"""
...
# Non-interactive backend (avoids Tk errors on Windows)
import matplotlib
matplotlib.use("Agg")
...
def build_stage1_segments(pure_red_signals: List[np.ndarray],
                          notred_signals: List[np.ndarray],
                          sr: float,
                          seg_len: int,
                          num_neg_per_notred: int = 4
                          ) -> Tuple[List[np.ndarray], np.ndarray,
                                     List[np.ndarray], List[np.ndarray]]:
    """
    Stage 1: build segments from pure RED and non-RED sentences.

    - For each pure-RED signal:
        * compute envelope
        * detect peaks (each peak ~ one RED)
        * extract a segment around each peak -> positive examples
    - For each non-RED sentence:
        * sample random segments -> negative examples
    ...
    # negatives from non-RED sentences
    for x in notred_signals:
        x_norm = normalize_signal(x)
        L = len(x_norm)
        if L < seg_len:
            continue
        for _ in range(num_neg_per_notred):
...
def create_stage2_segment_dataset(signals: List[np.ndarray],
    ...
    for idx in indices:
        ...
        if labels[idx] == 1:
            ...
            # negatives from same RED sentence (far from RED)
...
def run_pipeline(pure_red_zips: List[str],
    ...
    # ------------------------------------------------------------------
    # Stage 1: curriculum on pure RED vs non-RED
    # ------------------------------------------------------------------
    print("\n=== Stage 1: learning RED from pure repetitions ===")
    segments1, labels1, pos1, neg1 = build_stage1_segments(
        pure_red_signals=pure_signals,
        notred_signals=notred_sent_signals,
        sr=sr,
        seg_len=seg_len,
        num_neg_per_notred=4
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=40,
        help='Number of epochs for Stage-1 training (pure RED).'
    )
    parser.add_argument(
        '--epochs_stage2',
        type=int,
