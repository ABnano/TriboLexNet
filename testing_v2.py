...
# -*- coding: utf-8 -*-
"""
red_keyword_spotting_cnn.py  (window-tuned version)

Keyword spotting for the word "RED" in piezoelectric acoustic recordings.

Standard-style pipeline:
------------------------
1) Sentence-level audio -> fixed-length segments (~0.30 s).
...
This version uses a ~0.30 s window for segments (estimated from the
"Red 100x 10Hz" single-word recordings), which matches the actual
duration of a single RED event more closely and yields sharper
probability peaks than the previous 0.60 s window.
...
def build_initial_template(signals: List[np.ndarray],
    ...
    The default window_duration is set to 0.30 s, based on the
    single-word "Red 100x 10Hz" recordings, where individual RED
    bursts were found to last roughly 0.15–0.30 s.
    """
        ...
        if labels[idx] == 1:
            ...
            for _ in range(max(1, num_neg_per_sentence // 2)):
                for _try in range(20):
                    ...
                    # ensure far from RED anchor by at least seg_len
...
def sliding_window_probs(model: nn.Module,
                         ...
                         seg_len: int,
                         device: torch.device,
                         stride_sec: float = 0.02  # Finer stride -> better localization
    ...
    with torch.no_grad():
        for start in range(0, max(1, L - seg_len + 1), stride):
            ...
            X = torch.tensor(spec, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # (1,1,F,T)
            X = X.to(device)
...
def plot_detection_examples(signals: List[np.ndarray],
    ...
    threshold = 0.30  # tuned using max-probability histogram
    ...
    for row, idx in enumerate(chosen):
        ...
        # probs over time
        times, probs = sliding_window_probs(model, x, sr, seg_len, device, stride_sec=0.02)
        ax_p = axes[row, 1]
...
def plot_overlay_detected_segments(signals: List[np.ndarray],
    ...
    for idx in red_indices:
        ...
        # recover center index in samples
...
def run_pipeline(red_zip: str,
    ...
    # Sentence-level train/test split (we will only train CNN on train sentences)
    ...
    # window_duration now 0.30 s (tuned using single-word RED recordings)
    ...
    print(f"Created {len(seg_train_signals)} segments for training (positives + negatives).")
    print(f"Class balance: RED={seg_train_labels.sum()}, "
    ...
    model = train_cnn(model, train_loader, val_loader, device,
                      num_epochs=num_epochs, lr=1e-3, plots_dir=plots_dir)
    ...
    # Sentence-level prediction: label = 1 if max p(RED) > threshold
    threshold = 0.30  # chosen from histogram of max probabilities
    ...
    print("\nSentence-level performance (CNN max probability >= 0.30):")
    ...
    plot_detection_examples(signals, labels, metas, idx_test_sent, model, sr,
                            seg_len, device, plots_dir)
    plot_maxprob_hist(signals, labels, idx_test_sent, model, sr, seg_len,
                      device, plots_dir)
    plot_overlay_detected_segments(signals, labels, idx_test_sent, model, sr,
                                   seg_len, device, plots_dir)
...
def parse_args():
    ...
    parser.add_argument('--plots_dir', type=str, default='plots_red_cnn',
...
def main():
    args = parse_args()
    run_pipeline(args.red_zip, args.not_red_zip,
