...
Pipeline:
- Load two zip files:
    * one with sentences containing "RED" (e.g. Red_story_231125.zip)
    * one with sentences without "RED" (e.g. not_RED_speaking.zip)
- Preprocess signals.
- Implement four approaches:

  (1) Template / matched-filter baseline
      - Build a template for the word "RED" from "RED before story" and
        "RED after the story" recordings.
      - For each sentence, compute the maximum template correlation.
      - Train a logistic regression classifier on that single feature.

  (2) Hand-crafted spectral features + Random Forest
      - Extract simple time- and frequency-domain features.
      - Train a RandomForest classifier.
      - Report feature importance.

  (3) Log-power spectrogram + small 2D CNN (PyTorch)
      - Turn each signal into a log-power spectrogram.
      - Train a compact CNN to classify RED vs NOT-RED.

  (4) Raw waveform + 1D CNN (PyTorch)
      - Feed normalized 1D waveform into a small 1D CNN.

All approaches share the same train/test split for fair comparison.
"""
...
def build_red_template(signals: List[np.ndarray],
    ...
    """
    Build a template for the word "RED" using the sentences
    "RED before story" and "RED after the story".

    Steps:
      - Select all recordings whose sentence matches one of the target names.
      - For each, find the time index with maximum smoothed energy.
      - Extract a fixed-duration window around that centre.
      - Average all such windows to form the template.
      - Normalize the template (zero-mean, unit-norm).
    """
    ...
    for s, m in zip(signals, metas):
        ...
        # Simple energy envelope via moving average on |x|
        abs_x = np.abs(x)
...
def max_template_correlation(template: np.ndarray, signal: np.ndarray) -> float:
    """
    Compute a simple (not fully normalized) maximum correlation between
    the given template and the signal.

    Both inputs are assumed 1D arrays. The template is expected to be
    zero-mean and unit-norm.

    Returns:
        max_corr: scalar (float)
    """
...
def extract_handcrafted_features(signal: np.ndarray,
    ...
    """
    Extract simple time-domain and spectral features from a 1D signal.

    Features:
      - Time-domain stats for raw signal (mean, std, max, min, ptp, RMS)
      - Same for absolute signal
      - Stats for first difference (mean, std, max, min)
      - Binned spectral energy (n_bands bands from 0 to sr/2)
    ...
    # 4) Frequency-domain — magnitude spectrum binned into n_bands
...
def build_feature_matrix(signals: List[np.ndarray],
                         sr: float,
                         n_bands: int = 32) -> Tuple[np.ndarray, List[str]]:
    """Extract features for all signals and return (X, feature_names)."""
    feat_list = []
    names_ref: List[str] = []
...
def report_metrics(name: str,
    ...
    """
    Print a summary of performance for a binary classifier.
...
def train_torch_model(model: nn.Module,
    ...
    """Generic training loop for binary classification with BCE loss."""
...
def run_experiments(red_zip: str,
    ...
    # 2) Normalize and crop to common length
    ...
    meta_train = [metas[i] for i in idx_train]

    X_test = [signals[i] for i in idx_test]
    y_test = labels[idx_test]
    meta_test = [metas[i] for i in idx_test]

    print(f"Train size: {len(X_train)}, Test size: {len(X_test)}")

    # -----------------------------------------------------------
    # Approach 1 — Template + LogisticRegression
    # -----------------------------------------------------------
    if run_template:
        print("\n=== Running Approach 1: Template + LogisticRegression ===")
        template = build_red_template(signals, metas, sr)
        # single feature: max correlation with template
        ...
        report_metrics("Template + LogisticRegression", y_test, y_score)
    ...
    if run_rf:
        ...
        report_metrics("RandomForest + handcrafted features", y_test, y_score_rf)
    ...
    if run_cnn2d:
        ...
        # For deep models, carve out a validation split from the training data
        ...
        model2d = SmallCNN2D()
        model2d = train_torch_model(model2d, train_loader, val_loader, device, num_epochs=20, lr=1e-3)
        ...
        report_metrics("2D CNN on log-power spectrogram", y_true_2d, y_score_2d)
    ...
    if run_cnn1d:
        ...
        model1d = SmallCNN1D()
        model1d = train_torch_model(model1d, train_loader, val_loader, device, num_epochs=20, lr=1e-3)
        ...
        report_metrics("1D CNN on waveform", y_true_1d, y_score_1d)
